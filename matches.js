...
async function loadMatch(container) {
...
const match = data.events.find(e => e.id === FIXTURE_ID);  

if (!match) {  
  loadFromStorage("No match found for the specified match ID");  
  return;  
}  
...
if (match.status?.type?.state === "in") {  
  setTimeout(() => loadMatch(container), 30000);  
}
...
<!-- Advertisement -->  <ins class="adsbygoogle"  
style="display:block"  
...
</script></p></div>  <div class="teams-section">  
      <div class="logos-row">  
...
// Auto-init for all containers - Now uses data-fixture and data-league
document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll(".match-widget-container").forEach(container => {
      const fixtureId = container.getAttribute("data-fixture");
      const league = container.getAttribute("data-league");
      if (fixtureId) {
        loadMatch(container);
        setInterval(() => loadMatch(container), 30000);
      }
    });
});
